...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x_out = self.triplet_attention(x_out)
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes):
        ...
        #self.avgpool = ChannelAvgPool2d()
        self.fc = nn.Linear(512 * block.expansion, num_classes)
        #self.fc = nn.Linear(100 * block.expansion, num_classes)
    ...
    def forward(self, current):
        ...
        current = self.layer1(current) #layer1
        current = self.layer2(current) #layer2
        current = self.layer3(current) #layer3
        current = self.layer4(current) #layer4
            
        current_f_map = current
        current = self.avgpool(current)
        current = torch.flatten(current, 1)
        current = self.fc(current)

        return current, current_f_map
